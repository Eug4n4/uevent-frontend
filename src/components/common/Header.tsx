...
const IceLogo = () => (
  <svg className="ice-logo" viewBox="0 0 96 120" role="img" aria-label="nICE ice cube mascot">
    ...
    <rect x="12" y="12" width="64" height="56" rx="18" fill="url(#iceGradient)" stroke="#0d8fbd" strokeWidth="3" />
    <path
    ...
    <path d="M34 47c4 6 12 6 16 0" stroke="#ffffff" strokeWidth="4" strokeLinecap="round" fill="none" />
    <path d="M36 68c0 10 4 30-2 36" stroke="#0f7893" strokeWidth="5" strokeLinecap="round" fill="none" />
    <path d="M58 68c0 10 2 32 12 36" stroke="#0f7893" strokeWidth="5" strokeLinecap="round" fill="none" />
  ...
  return (
    <header className="hero-header">
      ...
      <nav>
        ...
        {isAuthenticated ? (
          <NavLink to={`/profile/${user?.id}`} className={({ isActive }) => `avatar-link ${isActive ? "active" : ""}`}>
            <UserAvatar />
          </NavLink>
        ) : (
          <NavLink to="/auth" className={({ isActive }) => `nav-cta ${isActive ? "active" : ""}`}>
            Login / Register
          </NavLink>
        )}
      </nav>
